...
TEST_CASE("01 - Testes de contorno") {
    ...
    // Testando argumentos negativos e iguais a zero
    std::vector <Localidade> vec_local; 
    std::ifstream arq_local("./data/localidades.txt");
    lerArquivoLocalidades(arq_local, vec_local);
    Operador operador1 = Operador(vec_local.size());

	CHECK_NOTHROW(operador1.popularMatriz());
    ...
    std::vector <Localidade> vetor_vazio; 

    CHECK_THROWS(Operador(vetor_vazio.size()));
	CHECK_NOTHROW(operador1.popularMatriz());
    CHECK_THROWS(operador1.dijkstra(-1, -1, -1, vetor_vazio)); // argumentos negativo
    CHECK_THROWS(operador1.dijkstra(0, 0, 0, vetor_vazio)); // argumentos com valor zero
    CHECK_THROWS(operador1.addSolicitacao(-1, -1, -1)); // argumentos negativo
    CHECK_THROWS(operador1.addSolicitacao(0, 0, 0)); // argumentos com valor zero
